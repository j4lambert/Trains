...
function specPicker(n:string,items:any[],value:string|number,f:Function,className?:string) {
return (
    <select
    name={n}
...
export function TrainDictPanel() {
    ...
    const [minStationLength, setMinStationLength] = useState(-1); const [maxStationLength, setMaxStationLength] = useState(-1);
    ...
    const [minStationLengthBool, setMinStationLengthBool] = useState(false); const [maxStationLengthBool, setMaxStationLengthBool] = useState(false);
    ...
    const [minStationLengthOperator, setMinStationLengthOperator] = useState(""); const [maxStationLengthOperator, setMaxStationLengthOperator] = useState("");
    ...
    const allNum:Record<string, AllNumEntry> = {
        ...
        "minStationLength": [minStationLength, setMinStationLength, minStationLengthBool, setMinStationLengthBool, minStationLengthOperator, setMinStationLengthOperator],
        "maxStationLength": [maxStationLength, setMaxStationLength, maxStationLengthBool, setMaxStationLengthBool, maxStationLengthOperator, setMaxStationLengthOperator],
    ...
    function numericFilterCond(value:number,key:string,operator:string) {
        const trainKey:keyof c.Train = key as keyof typeof Trains[0];
    ...
    useEffect(() => {
        ...
        out = Trains.filter(t => {
            return holdNum.every(([key,value]) => {
                return numericFilterCond(value[0],key as keyof c.Train,value[4])
            })
        })
    ...
    function registrationPreview() {
        ...
        const values:Partial<Record<keyof typeof all,c.TrackGauge|c.LoadingGauge|c.Electrification|c.PowerSupply|c.TrainType|c.AutomationLevel>> = p.getAll(temp);
    ...
    function inputWithModeAndOperator(
        ...
        state: boolean,
        setState: Function,
        classN: string = "flex items-center flex-1 gap-1 leading-loose"
    ) {
        return (
        <div className={classN}>
    ...
    return (
        ...
        <div className="flex justify-between gap-2 w-full">
            {pickerWithMode(specPicker("Automation Standard",als,auto,setAuto),autoBool,setAutoBool)}
            {pickerWithMode(specPicker("Electrification Standard",es,elect,setElect),electBool,setElectBool)}
            {pickerWithMode(specPicker("Track Gauge",tgs,gauge,setGauge),gaugeBool,setGaugeBool)}
        </div>
        <div className="flex justify-between gap-2 w-full">
            {pickerWithMode(specPicker("Loading Gauge",lgs,width,setWidth),widthBool,setWidthBool)}
            {pickerWithMode(specPicker("Power Supply",pss,power,setPower),powerBool,setPowerBool)}
            {pickerWithMode(specPicker("Train Type",tts,type,setType),typeBool,setTypeBool)}
        </div>
        <div className="flex justify-between gap-2 w-full">
            {pickerWithMode(specPicker("Minimum Station Length",lens,min,setMin),minBool,setMinBool)}
            {pickerWithMode(specPicker("Maximum Station Length",lens,max,setMax),maxBool,setMaxBool)}
        </div>
        <div className="flex justify-between gap-2 w-full">
            {pickerWithMode(specPicker("Minimum Station Length",lens,min,setMin),minBool,setMinBool)}
            {pickerWithMode(specPicker("Maximum Station Length",lens,max,setMax),maxBool,setMaxBool)}
